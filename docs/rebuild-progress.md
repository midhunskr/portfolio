...
- **Open Graph image**: audited first — confirmed no OG image existed anywhere (no file in `public/`, no `app/opengraph-image.*`, no `images` field in either metadata block) before creating one. Built `public/og-image.jpg` (1200×630, 48KB) by rendering the actual design as real HTML with real Google-hosted webfonts (Bricolage Grotesque/Hanken Grotesk/JetBrains Mono, matching the site's own font choices) in a scratch page, then exporting via `html2canvas` in-browser and decoding the resulting data URL to a file with a one-off Node script — chosen specifically to avoid `next/og`'s `ImageResponse`, which is the same broken dependency that blocked the favicon earlier in this phase. The design reuses the homepage's actual hero copy verbatim ("I design, build & automate products.", with the same green/amber accent words), the exact Navbar MS-badge lockup, and the same dot-grid + soft blob background treatment as the Hero section — not a generic template. `data/site.js` gained a single `ogImage` object (`url`/`width`/`height`/`alt`) referenced from `app/layout.jsx`, `app/privacy/page.jsx`, and `app/terms/page.jsx` — all three needed it explicitly, since Next's metadata merging replaces a route's entire `openGraph` object rather than deep-merging when a child segment defines its own. Verified: exact 1200×630 via JPEG header parse, `og:image`/`twitter:image`/`og:image:width`/`og:image:height`/`og:image:alt` all confirmed correct on `/`, `/privacy`, and `/terms` via direct DOM query, zero console errors, clean lint, clean production build.
