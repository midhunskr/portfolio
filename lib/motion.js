...
/**
 * Index-based hero reveal variant. The reference staggers each hero element
 * by DOM order (setTimeout at 120 + i*110ms) regardless of nesting, so the
 * three headline lines animate individually even though they sit inside one
 * <h1>. Encoding the delay per-index reproduces that exactly and side-steps
 * container-stagger propagation through non-motion wrappers.
 *
 * @param {number} [i=0] Zero-based position in the reveal sequence.
 * @returns {import('framer-motion').Variants}
 */
export function heroReveal(i = 0) {
  return {
    hidden: { opacity: 0, y: 34 },
    shown: {
      opacity: 1,
      ...
      transition: {
        duration: DUR.hero,
        ease: EASE,
        delay: STAGGER.hero.delay + i * STAGGER.hero.children,
      },
    },
  };
}
